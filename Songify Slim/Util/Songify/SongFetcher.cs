...
public class SongFetcher
{
    ...
    public async Task FetchDesktopPlayer(string player)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                Util.General.AppShellBridge.Current?.SetStatusText("Error uploading Song information");
            }));
        }

        Application.Current.Dispatcher.Invoke(() =>
        {
            Util.General.AppShellBridge.Current?.SetTextPreview(output);
        });
    }
    ...
    private static async Task ExecutePauseActions()
    {
        switch (Settings.PauseOption)
        {
            ...
            case Enums.PauseOptions.PauseText:
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    Util.General.AppShellBridge.Current?.SetTextPreview(Settings.CustomPauseText);
                });
                break;
            ...
            case Enums.PauseOptions.ClearAll:
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    Util.General.AppShellBridge.Current?.SetTextPreview("");
                });
                break;
    ...
    private static async Task WriteSongInfo(TrackInfo songInfo, Enums.RequestPlayerType playerType = Enums.RequestPlayerType.Other)
    {
        if (!songInfo.IsPlaying)
        {
            ...
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                {
                    case Enums.PauseOptions.PauseText:
                        shell.SetTextPreview(Settings.CustomPauseText);
                        break;

                    case Enums.PauseOptions.ClearAll:
                        shell.SetTextPreview("");
                        break;
            ...
            currentSongOutput.Trim() != Settings.CustomPauseText)
        {
            ...
            catch (Exception ex)
            {
                ...
                Application.Current.Dispatcher.Invoke(DispatcherPriority.Normal, new Action(() =>
                {
                    Util.General.AppShellBridge.Current?.SetStatusText("Error uploading history");
                }));
            }
        }
        ...
        Application.Current.Dispatcher.Invoke(() =>
        {
            string text = currentSongOutput.Trim();
            if (text.StartsWith("-"))
                text = text.Remove(0, 1).Trim();
            Util.General.AppShellBridge.Current?.SetTextPreview(text.Replace(@"\n", " - ").Replace("  ", " "));
        });
    }
