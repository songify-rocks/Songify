...
public static class SpotifyApiHandler
{
    ...
    private static async Task ApplyAuthenticatedStateAsync(PKCETokenResponse tokenResponse)
    {
        ...
        await app.Dispatcher.InvokeAsync(async () =>
        {
            try
            {
                ...
                if (shell != null)
                    shell.SetSpotifyState(Util.General.SpotifyIndicatorState.Premium);
                ...
                if (GlobalObjects.SpotifyProfile.Product != "premium")
                {
                    ...
                    if (shell != null)
                        shell.SetSpotifyState(Util.General.SpotifyIndicatorState.Free);
                }
    ...
    public static void LogoutSpotify()
    {
        try
        {
            ...
            Util.General.AppShellBridge.Current?.SetSpotifyState(Util.General.SpotifyIndicatorState.Disconnected);
        }
        catch (Exception ex)
        {
            Logger.LogExc(ex);
