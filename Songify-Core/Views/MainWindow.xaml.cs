...
namespace Songify_Slim.Views
{
    public partial class MainWindow
    {
        ...
        private async void MetroWindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (_selectedSource == PlayerType.SpotifyWeb)
            {
                if (string.IsNullOrEmpty(Settings.SpotifyAccessToken) && string.IsNullOrEmpty(Settings.SpotifyRefreshToken))
                    TxtblockLiveoutput.Text = Songify_Core.Properties.Resources.mw_LiveOutputLinkSpotify;
                else
                    await ApiHandler.Auth();
        ...
        //                TwitchHandler.MainConnect();
        //            }),
        //            new MenuItem("Disconnect", (sender1, args1) => { TwitchHandler.Client.Disconnect(); })
        //        }),
        ...
        {
            Settings.IsLive = await TwitchHandler.CheckStreamIsUp();
            mi_TwitchCheckOnlineStatus.Header = $"{Songify_Core.Properties.Resources.mw_menu_Twitch_CheckOnlineStatus} ({(Settings.IsLive ? "Live" : "Offline")})";
