...
namespace Songify_Slim.Util.Songify
{
    ...
    public static class TwitchHandler
    {
        ...
        private static async void AddSong(string trackId, OnMessageReceivedArgs e)
        {
            ...
            SearchResponse? search = await ApiHandler.SearchForItem(SearchRequest.Types.Track, trackId);
            if (search == null)
                return;

            FullTrack? track = search.Tracks.Items?[0];
            ...
            bool success = await ApiHandler.AddToQueue("spotify:track:" + trackId);
            if (!success)
            {
        ...
        {
            FullPlaylist? playlist = await ApiHandler.GetPlaylistAsync(spotifySongLimitPlaylist);
            ...
            return Tuple.Create(false, playlist);
        }
        ...
        private static async Task<ReturnObject> AddSong2(string trackId, string username)
        {
            ...
            FullTrack? track = await ApiHandler.GetTrackById(trackId);
            if (track == null)
            {
                return new ReturnObject
                {
                    Msg = "No track was found.",
                    Success = false,
                    Refundcondition = 3
                };
            }

            if (track?.IsPlayable != null && !track.IsPlayable)
            {
                return new ReturnObject
                {
            ...
            bool success = await ApiHandler.AddToQueue(spotifyUri);
            if (!success)
            {
                // if an error has been encountered, log it, inform the requester and skip
                Logger.LogStr("Spotify: Error adding to queue");
                response = Settings.Settings.BotRespError;
                response = response.Replace("{user}", username);
                response = response.Replace("{artist}", "");
                response = response.Replace("{title}", "");
                response = response.Replace("{maxreq}", "");
                response = response.Replace("{errormsg}", "There was an issue adding to the queue");
        ...
        private static async Task<bool> AddToPlaylist(string trackId, bool sendResponse = false)
        {
            try
            {
                ...
                await ApiHandler.AddSongToPlaylist(Settings.Settings.SpotifyPlaylistId, $"spotify:track:{trackId}");
        ...
        private static async void Client_OnMessageReceived(object sender, OnMessageReceivedArgs e)
        {
            ...
            if (Settings.Settings.Player == 0 && e.ChatMessage.Message == $"!{Settings.Settings.BotCmdSkipTrigger}" && Settings.Settings.BotCmdSkip)
            {
                    ...
                    (count > 0 && name == e.ChatMessage.DisplayName))
                {
                    ...
                    bool success = await ApiHandler.SkipCurrentSong();
                    if (!success)
                    {
                        SendChatMessage(e.ChatMessage.Channel, "Error skipping song");
                    }
                    else
                    {
                        if (msg.StartsWith("[announce "))
        ...
        private static async Task<string> GetTrackIdFromInput(string input)
        {
            ...
            SearchResponse? searchItem = await ApiHandler.SearchForItem(SearchRequest.Types.Track, HttpUtility.UrlEncode(input));

            if (searchItem == null)
            {
                SendChatMessage(Settings.Settings.TwChannel, "Error searching for specified query");
                return "";
            }
            ...
            OnChannelPointsRewardRedeemedArgs e)
        {
            ...
            if (reward.Id == Settings.Settings.TwRewardSkipId)
            {
                ...
                if (!success)
                {
                    SendChatMessage(Settings.Settings.TwChannel, "Error skipping song");
                }
                else
                {
            ...
            if (reward.Id == Settings.Settings.TwRewardGoalRewardId)
            {
                ...
                if (GlobalObjects.RewardGoalCount >= Settings.Settings.RewardGoalAmount)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        bool success = await ApiHandler.AddToQueue($"spotify:track:{songId}");
                        if (success)
                            await ApiHandler.SkipCurrentSong();
                    }
                }
            }
        }
